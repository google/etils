...
from etils.array_types import typing as array_typing
...
@typing.overload
def check_arrays(fn: None = ..., *, strict: bool = ...) -> Callable[[_Fn], _Fn]:
  ...


@typing.overload
def check_arrays(fn: _Fn = ..., *, strict: bool = ...) -> _Fn:
  ...


def check_arrays(fn=None, *, strict: bool = True):
  """Check.
  ...
  if fn is None:
    return functools.partial(check_arrays, strict=strict)
...
def _get_array_param(
  ...
  def make_err(msg: str) -> Exception:
    return NotImplementedError(
        f'`enp.check_arrays` does not support {msg}. Please open an issue if '
        f'you need this feature. For `{name}: {hint}`')
