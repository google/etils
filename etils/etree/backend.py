...
class Python(Backend):
  ...
  def map(self, map_fn, *trees):
    tree0 = trees[0]
    if isinstance(tree0, (list, tuple)):
      return type(tree0)(self.map(map_fn, *v) for v in zip(*trees))
    elif isinstance(tree0, (dict, collections.abc.Mapping)):
      return type(tree0)(
  ...
  def _flatten(self, tree):
    ...
    elif isinstance(tree, (dict, collections.abc.Mapping)):
  ...
  def _unflatten(self, structure, flat_iter):
    ...
    elif isinstance(structure, (dict, collections.abc.Mapping)):
  ...
  def _assert_same_structure(
    ...
    # TODO(epot): Could wrap iteration in `epy.reraise` for better debugging
    if isinstance(tree0, (list, tuple, dict, collections.abc.Mapping)):
