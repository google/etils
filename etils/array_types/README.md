...
Functions input/output can be annotated to help the reader better understand
...
Note: Those typing annotations not (yet) detected by static type checking tools.
But are already helpful as documentation.
...
*   DType can be:
    ...
    *   `FloatArray` (accept `f32`, `bf16`,...), `IntArray` (accept `ui8`,
...
You can decorate your function with `@enp.check_arrays` so that array
shape/dtype are dynamically validated at runtime:
...
Function decorated with `enp.check_arrays` support both `np`, `jnp`, `tnp`:

*   If args are both `jnp` and `tnp`, an error is raised
*   If args are `xnp` with `np`, the `np` array is auto-casted
    to xnp.
...
There are 2 level of checking:

*   Using type union: `IntArray` (accept `ui8`, `i32`, `i64`,...), `FloatArray`
    (accept `f32`, `bf16`,...)
*   Using specific type: `f32`, `ui8`,...

Using type unions allow your functions to support quantization,...
